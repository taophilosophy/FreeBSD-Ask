...
首先不要被 NixOS 所提出的一系列名词弄花了眼睛，那些没有用而且是忽悠人的。

从根本上看，NixOS 就是把 Node.js 做成了一款系统，因此他在所有方面都和 Node.js 是一样的，是一样而非接近。

NixOS 宣传的可重现、包括配置文件的写法、软件包的锁定（使用 SHA 锁定依赖）、再到他的更新方式，下载方式，甚至报错都和 Node.js 一个毛病，不老实报错：一个包没有就你说没有，非说语法有错误。
